...
class DQNAgent:
    def __init__(self, state_size, hidden_size, action_size, u_min, u_max, lr, gamma, soft_update_tau, epsilon, eps_decay, min_epsilon, ep_max_nominal, ev_max_nominal, acc_min, acc_max, buffer_size, batch_size, device='cpu', discrete_actions=False):
...
class TabularQAgent:
    def __init__(self, state_bins, num_actions, u_min, u_max, lr, gamma, epsilon, eps_decay, min_epsilon):
        ...
        # Creiamo una lista di azioni discrete
    ...
    def select_action(self, state):
        if np.random.random() < self.epsilon:
            ...
            # Scegliamo l'azione con il valore Q più alto
            action_index = np.argmax(self.q_table[state])

        # Restituiamo il valore effettivo dell'azione
        return self.discrete_actions[action_index]

    def update(self, state, action, reward, next_state, done):
        # Troviamo l'indice dell'azione più vicina a quella presa
